...
The installer detects your platform, installs the binary, puts the daemon
on login, wires DNS for dev zones, and installs the TLS trust. Everything
it writes is reversible and marker-tagged. If you prefer brew,
`brew install nano112/tap/gerry && gerry bootstrap` does the same. Then:
...
- **Kubernetes**: an observer that audits what your ingress actually
  ...
  that materializes routes (Traefik or Gateway API) from allocations. It
  only ever touches resources it labeled itself, and a CI e2e proves that
  on every change.
...
- [Scoped tokens](tokens.md): multi-tenant credentials and the audit trail
